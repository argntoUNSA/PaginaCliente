...
branch : nos permite enlistar, crear, renombrar y borrar ramas
	parametros : 
		<> : enlista todas las ramas existentes, marcando con color diferente la 
			cual este siendo usada actualmente.
		<nombre de la rama> : Crea una nueva rama con el nombre señalado.
		-m <nombre old> <nombre new> : cambia el nombre de la rama llamada <nombre old>
						llamandola ahora <nombre new>
		-d <nombre de la rama> : Elimina todas las ramas excepto la del nombre indicado
		-d <> : Elimina todas las ramas excepto la que esta usando actualmente.
		-f <nombre de la rama>: fuerza la creacion, el traslado/renombramiento o la 
					eliminacion de alguna rama. 
...
aun no lo este, en algunas versiones funciona como el comando 'restore'
	parametros:
		<nombre de la rama> : Se mueve a la rama indicada si es que esta creada.
